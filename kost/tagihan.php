...
$query = "SELECT * ,DATE_ADD(tagihan.tanggal_tagihan, INTERVAL 90 DAY) as jatuh_tempo FROM tagihan JOIN booking ON tagihan.no_booking=booking.id_booking JOIN user ON user.id=booking.id_user JOIN kost ON booking.id_kost=kost.id_kost WHERE user.username ='$username'";
